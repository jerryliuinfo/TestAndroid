...
public abstract class WorkTask<Params, Progress, Result> {
	...
	public enum Status {
		/**
		 * Indicates that the mTask has not been executed yet.
		 */
		PENDING,
		/**
		 * Indicates that the mTask is running.
		 */
		RUNNING,
		/**
		 * Indicates that {@link WorkTask#onPostExecute} has finishe.
		 */
		FINISHED,
	}
	...
	public final WorkTask<Params, Progress, Result> executeOnExecutor(Executor exec, Params... params) {
		if (mStatus != Status.PENDING) {
			switch (mStatus) {
				case RUNNING:
					throw new IllegalStateException("Cannot execute mTask:" + " the mTask is already running.");
				case FINISHED:
					throw new IllegalStateException("Cannot execute mTask:" + " the mTask has already been executed "
							+ "(a mTask can be executed only once)");
			}
		}
